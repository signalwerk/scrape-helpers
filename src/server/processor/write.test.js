...
describe("urlToPath", () => {
  test("handles basic URL with html mime type", async () => {
    ...
    expect(result).toBe("/example.com/page.html");
  });

  test("handles URL ending with slash", async () => {
    const job = "https://example.com/folder/";
    const result = await urlToPath(job, "text/html");
    expect(result).toBe("/example.com/folder/index.html");
  });
  ...
  test("handles very long urls", async () => {
    ...
    const result = await urlToPath(job, "text/html");
    expect(result).toBe(
      "/example.com/äöü/this_is_a_very_long_test_filename_designed_to_test_the_write_function_that_requires_handling_of_extremely_long_file_names_and_we_are_including_various_characters_numbers_and_more_just_to_fill_up_the_space_and_see_how_it_handles_extreme.html",
    );
  });
  ...
  test("adjust http to https", async () => {
    ...
    const result = await urlToPath(job, "text/html");
    expect(result).toBe("/example.com/file.html");
  });
});
